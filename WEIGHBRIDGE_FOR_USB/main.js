...
function createWindow() {
    ...
    ipcMain.handle('run-local-ai', async (event, base64Image) => {
        sendLog('Memulai Gemini AI...');
            ...
            try {
                ...
                const requestBody = {
                    contents: [
                        {
                            parts: [
                                { text: "Anda adalah sistem ALPR (Automated License Plate Recognition) untuk plat nomor Indonesia. Ekstrak nomor plat dari gambar ini. HANYA kembalikan teks nomor plat (contoh: B 1234 ABC). Jangan tambahkan teks lain." },
                                {
                                    inline_data: {
                ...
                if (!response.ok) {
                    ...
                    sendLog(`Gemini API Error: ${response.status} - ${errorText}`);
                    return resolve({ success: false, error: `Gemini API Error: ${response.status}` });
                }

                const data = await response.json();
                
                if (data.candidates && data.candidates.length > 0 && data.candidates[0].content && data.candidates[0].content.parts.length > 0) {
                    let plate = data.candidates[0].content.parts[0].text.trim();
                    plate = plate.replace(/\n/g, '').trim();
                    sendLog(`Gemini AI Berhasil: ${plate}`);
                    
                    // Trigger webhook to PythonAnywhere
                    const PRODUCTION_API = 'https://sabrent.pythonanywhere.com/api/webhooks/weighbridge';
                    // We mock gross/tare for now since we only have the plate in this AI function
                    const payload = {
                        plate: plate,
                        gross: 0, 
                        tare: 0,
                    ...
                    .then(r => r.json())
                    .then(resData => sendLog(`Webhook Terkirim ke Cloud: ${JSON.stringify(resData)}`))
                    .catch(err => sendLog(`Error Webhook Cloud: ${err.message}`));
    ...
    // =========================================================================
    function initScalePort(retryCount = 0) {
        ...
        try {
            scalePort = new SerialPort({ 
                ...
                stopBits: 1,
                parity: 'none',
                autoOpen: false // We will open it manually to catch immediate errors
            });
            ...
            scalePort.open((err) => {
                if (err) {
                    ...
                    // Fallback: list available ports to help troubleshooting
    ...
    // Initialize scale after window is ready
    setTimeout(() => initScalePort(), 2000);

    // Allow manual retry from renderer
